...
function parseInputDate(dateStr) {
  ...
  if (isNaN(y) || isNaN(m) || isNaN(d)) {
    return new Date(); // fallback if string was malformed
  }
...
function generateBillOccurrences(periodStart, periodEnd, billAnchor, billFrequency) {
  let occurrences = [];
  let current = new Date(billAnchor);
...
function saveToLocalStorage() {
  localStorage.setItem("bills", JSON.stringify(bills.map(b => ({
    name: b.name,
    amount: b.amount,
    frequency: b.frequency,
    anchor: b.anchor.toISOString().split("T")[0]
  }))));
...
function loadFromLocalStorage() {
  ...
  bills = savedBills.map(b => ({
    name: b.name,
    amount: b.amount,
    frequency: b.frequency,
    anchor: b.anchor ? parseInputDate(b.anchor) : new Date()
  }));
...
function updateBill(index, field, value) {
  if (field === "anchor") {
    value = value ? parseInputDate(value) : new Date();
  }
...
function addEmptyBillRow() {
  bills.push({ name: "", amount: 0, frequency: "monthly", anchor: new Date() });
  renderBills();
}
...
document.getElementById("saveIncome").onclick = () => {
  saveToLocalStorage();
  const msg = document.getElementById("incomeSaved");
...
document.getElementById("saveBills").onclick = () => {
  saveToLocalStorage();
  const msg = document.getElementById("billsSaved");
  msg.style.display = "inline";
  setTimeout(() => msg.style.display = "none", 1000);
